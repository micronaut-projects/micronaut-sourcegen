...
When a generated class references a type that is still being compiled, the backend falls back to
compiling Java source and resolves such types from `src/main/java` and `src/test/java` of the project
directory. Sources kept elsewhere, such as shared source sets, can be added through the
`micronaut.sourcegen.bytecode.jdk.sourcepath` annotation-processor option, which accepts a
path-separator-delimited list of source roots:
