...
The backend writes class files directly for top-level classes and records. Interfaces, enums, nested
types and constructs the direct writer does not lower yet are rendered to Java source and compiled with
the in-process JDK compiler. Should that compilation fail as well, the generator emits the Java source
file and reports a compiler warning naming the class and the reason.
