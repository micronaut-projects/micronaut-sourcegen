...
class KotlinSourceCompilationTest {
    ...
    fun superConstructorWithExplicitSuperType() {
        ...
        val classDefSource = render(classDef)
        assertEquals(
            """
            |package test
            |
    ...
    fun fieldOfAnotherType() {
        ...
        val accessorSource = render(accessor)
        assertEquals(
            """
            |package test
            |
    ...
    fun blankFieldOfABoxedPrimitiveType() {
        ...
        val count = FieldDef.builder("count", Integer::class.java)
            ...
            .build()
        // No constructor assigns the field, which holds null, the JVM default of the boxed type, until the initializer runs
    ...
    fun anyReturnedAsATypeVariableOrAnArray() {
        ...
        val stringsSource = render(strings)
        assertEquals(
            """
            |package test
            |
    ...
    fun returnErasedToABoundIsCast() {
        ...
        val boundedSource = render(bounded)
        assertEquals(
            """
            |package test
            |
            ...
            |
            |public fun interface Bounded<T : CharSequence> {
            |  public fun `get`(): T
            |}
            |""".trimMargin(),
            boundedSource
    ...
    fun constructorAssigningTheFieldOfAnotherInstance() {
        ...
        // `other.name` leaves `this.name` unassigned: it holds null, the JVM default
    ...
    fun assignmentAfterAReturnOfTheConstructor() {
        ...
        val name = FieldDef.builder("name", String::class.java)
            .addModifiers(Modifier.PUBLIC)
            .build()
        // The assignment after the return is never reached, and not rendered: the property holds null, the JVM default
    ...
    fun narrowedArrayReturnIsCast() {
        ...
        assertEquals(
            ...
            |import kotlin.CharSequence
            |
            |public fun interface BoundedArray<T : CharSequence> {
    ...
    fun anyValueOfAPrimitiveTarget() {
        val take = MethodDef.builder("take").addModifiers(Modifier.PRIVATE)
            ...
            .build { _, _ -> StatementDef.multi() }
        // An Any value is passed to an Int parameter, and returned from an Int method: unboxed through Number, as the
        // bytecode does, which converts any number
        ...
        assertEquals(
            ...
            |    this.take((`value` as Number).toInt())
            |  }
            |
    ...
    fun callOfAnInheritedNarrowedMethodIsConverted() {
        ...
        val parentSource = render(parent)
        assertEquals(
            """
            |package test
            |
    ...
    fun referenceThroughAFieldReadsItOnce() {
        ...
        val targetSource = render(target)
        assertEquals(
            """
            |package test
            |
