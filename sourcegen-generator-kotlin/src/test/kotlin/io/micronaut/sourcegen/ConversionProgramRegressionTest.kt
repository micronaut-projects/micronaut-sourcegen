...
class ConversionProgramRegressionTest {
    @TestFactory
    fun arrayHelperPreservesCallerVariables() = listOf("T", "R").map { name ->
        dynamicTest(name) {
    ...
    @TestFactory
    fun inheritedCallsMatchTheFullGenericSignature() = listOf("overload", "array").map { kind ->
        dynamicTest(kind) {
    ...
    @TestFactory
    fun successiveCastsKeepEveryBound() = listOf("scalar", "array", "recursive").map { kind ->
        dynamicTest(kind) {
    ...
    @TestFactory
    fun composedSmartCastsPreserveOverloadSelection() = listOf("parameter", "conditional", "switch").map { kind ->
        dynamicTest(kind) {
            val v = TypeDef.variable("V", TypeDef.of(Serializable::class.java))
            val first = MethodDef.builder("serializable").addModifiers(Modifier.PUBLIC, Modifier.STATIC).addTypeVariable(v)
                .addParameter("value", v).returns(TypeDef.OBJECT).build()
    ...
    private fun compile(vararg definitions: ObjectDef): URLClassLoader {
        val sources = definitions.map { definition ->
            ...
            val resource = "/conversion-programs/kotlin/${definition.simpleName}.txt"
