...
class GeneratedProgramTest {
    ...
    @TestFactory
    fun genericArrayCallsPreserveValuesAcrossRanks() = (1..3).map { rank ->
        DynamicTest.dynamicTest("rank $rank") {
    ...
    @TestFactory
    fun compatibleArgumentsKeepTheirValues() = listOf("number", "text", "list").map { kind ->
    ...
    @TestFactory
    fun referencesCaptureTheirReceiverBeforeItChanges() = listOf("field", "local", "result").map { kind ->
        DynamicTest.dynamicTest(kind) {
            ...
            compile(target, caller).use { loader ->
    ...
    private fun compile(vararg definitions: ObjectDef): URLClassLoader {
        val sources = definitions.map { definition ->
            ...
            val resource = "/generated-programs/kotlin/${definition.simpleName}.txt"
