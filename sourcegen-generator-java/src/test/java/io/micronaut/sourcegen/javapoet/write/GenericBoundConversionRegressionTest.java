...
class GenericBoundConversionRegressionTest {
    ...
    @Test
    void returnConversionFollowsLongBoundChain() throws Exception {
        var method = MethodDef.builder("call").addModifiers(Modifier.PUBLIC);
        TypeDef bound = TypeDef.parameterized(List.class, Object.class);
        for (int i = 10; i >= 0; i--) {
            var variable = TypeDef.variable("T" + i, bound);
            method.addTypeVariable(variable);
            bound = TypeDef.variable(variable.name());
        }
        var def = ClassDef.builder("test.LongBoundChain").addModifiers(Modifier.PUBLIC)
            .addMethod(method.addParameter("value", TypeDef.parameterized(List.class, String.class))
                .returns(bound).build((s, p) -> p.getFirst().returning())).build();
        try (var loader = compile(def)) {
            var cls = loader.loadClass(def.getName());
            var value = List.of("hello");
            assertSame(value, cls.getMethod("call", List.class).invoke(cls.getConstructor().newInstance(), value));
        }
    }
}
