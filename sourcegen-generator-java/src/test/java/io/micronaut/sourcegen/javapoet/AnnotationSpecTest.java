...
public final class AnnotationSpecTest {
  ...
  void defaultAnnotation(CompilationRule compilation) {
    ...
    assertThat(toString(taco)).isEqualTo("package com.squareup.tacos;\n"
        + "\n"
        + "import io.micronaut.sourcegen.javapoet.AnnotationSpecTest;\n"
  ...
  void defaultAnnotationWithImport(CompilationRule compilation) {
    ...
    JavaFile file = JavaFile.builder("io.micronaut.sourcegen.javapoet", typeBuilder.build()).build();
  ...
  void emptyArray() {
    ...
    assertThat(builder.build().toString()).isEqualTo(
        "@io.micronaut.sourcegen.javapoet.AnnotationSpecTest.HasDefaultsAnnotation(" + "n = {}" + ")");
    builder.addMember("m", "$L", "{}");
    assertThat(builder.build().toString())
        .isEqualTo(
            "@io.micronaut.sourcegen.javapoet.AnnotationSpecTest.HasDefaultsAnnotation("
                + "n = {}, m = {}"
                + ")");
  }

  @Test
  void dynamicArrayOfEnumConstants() {
    AnnotationSpec.Builder builder = AnnotationSpec.builder(HasDefaultsAnnotation.class);
    builder.addMember("n", "$T.$L", Breakfast.class, Breakfast.PANCAKES.name());
    assertThat(builder.build().toString()).isEqualTo(
        "@io.micronaut.sourcegen.javapoet.AnnotationSpecTest.HasDefaultsAnnotation("
            + "n = io.micronaut.sourcegen.javapoet.AnnotationSpecTest.Breakfast.PANCAKES"
            + ")");
    ...
    assertThat(builder.build().toString()).isEqualTo(
        "@io.micronaut.sourcegen.javapoet.AnnotationSpecTest.HasDefaultsAnnotation("
            + "n = {"
            + "io.micronaut.sourcegen.javapoet.AnnotationSpecTest.Breakfast.PANCAKES"
            + ", io.micronaut.sourcegen.javapoet.AnnotationSpecTest.Breakfast.WAFFLES"
            + ", io.micronaut.sourcegen.javapoet.AnnotationSpecTest.Breakfast.PANCAKES"
            + "})");
  ...
  void defaultAnnotationToBuilder(CompilationRule compilation) {
    ...
    assertThat(builder.build().toString()).isEqualTo(
        "@io.micronaut.sourcegen.javapoet.AnnotationSpecTest.HasDefaultsAnnotation("
            + "o = io.micronaut.sourcegen.javapoet.AnnotationSpecTest.Breakfast.PANCAKES"
            + ", p = 1701"
            + ", f = 11.1"
            + ", m = {9, 8, 1, 123}"
            + ", l = java.lang.Override.class"
            + ", j = @io.micronaut.sourcegen.javapoet.AnnotationSpecTest.AnnotationA"
            + ", q = @io.micronaut.sourcegen.javapoet.AnnotationSpecTest.AnnotationC(\"bar\")"
            + ", r = {java.lang.Float.class, java.lang.Double.class}"
            + ")");
  }

  @Test
  void reflectAnnotation() {
  ...
  @Test
  void reflectAnnotationWithDefaults() {
    HasDefaultsAnnotation annotation = IsAnnotated.class.getAnnotation(HasDefaultsAnnotation.class);
