...
import static io.micronaut.sourcegen.JavaExpressionRules.isNegativeNumericConstant;
import static io.micronaut.sourcegen.JavaExpressionRules.isNullLiteral;
import static io.micronaut.sourcegen.JavaExpressionRules.creationComponent;
import static io.micronaut.sourcegen.JavaExpressionRules.asPrimitiveOperand;
...
import static io.micronaut.sourcegen.JavaPoetNames.asClassType;
...
import static io.micronaut.sourcegen.JavaSourceRules.validateFieldAccess;
...
public sealed class JavaPoetSourceGenerator implements SourceGenerator permits GroovyPoetSourceGenerator {
    ...
    /**
     * The label of the block an initializer that returns is written in. A label shares no namespace with a local.
     */
    private static final String INITIALIZER_LABEL = "init";

    private final JavaConversionRenderer conversions = new JavaConversionRenderer(this);
    ...
    public void write(ObjectDef objectDef, Writer writer) throws IOException {
        ObjectDef previous = OverrideResolver.writing(objectDef);
        try {
            switch (objectDef) {
            ...
            OverrideResolver.writing(previous);
        }
    }
    ...
    private TypeSpec.Builder getInterfaceBuilder(InterfaceDef interfaceDef) {
        ...
        interfaceDef.getJavadoc().forEach(javadoc -> interfaceBuilder.addJavadoc("$L", javadoc));
        ...
        for (PropertyDef property : interfaceDef.getProperties()) {
            // An interface declares no field: a property is its abstract accessors, as the bytecode writer declares
            // them, and the property's javadoc and annotations are those of its getter
            TypeName propertyType = asType(property.getType(), interfaceDef);
            String propertyName = property.getName();
            Modifier[] modifiers = JavaSourceRules.interfacePropertyModifiers(property);
            String capitalizedPropertyName = NameUtils.capitalize(propertyName);
            MethodSpec.Builder getter = MethodSpec.methodBuilder("get" + capitalizedPropertyName)
                .addModifiers(modifiers)
                .returns(propertyType);
            property.getJavadoc().forEach(javadoc -> getter.addJavadoc("$L", javadoc));
            for (AnnotationDef annotation : property.getAnnotations()) {
                getter.addAnnotation(asAnnotationSpec(annotation));
            }
            interfaceBuilder.addMethod(getter.build());
    ...
    private TypeSpec.Builder getEnumBuilder(EnumDef enumDef) {
        ...
        enumDef.getJavadoc().forEach(javadoc -> enumBuilder.addJavadoc("$L", javadoc));
        ...
        enumDef.getEnumConstants().forEach(e -> {
            ...
            TypeSpec.Builder constant = type;
            e.javadoc().forEach(javadoc -> constant.addJavadoc("$L", javadoc));
            enumBuilder.addEnumConstant(e.name(), constant.build());
        });
    ...
    private TypeSpec.Builder getAnnotationObjectBuilder(AnnotationObjectDef def) {
        ...
        def.getJavadoc().forEach(javadoc -> builder.addJavadoc("$L", javadoc));
        ...
        for (AnnotationMemberDef member: def.getMembers()) {
            ...
            member.getJavadoc().forEach(javadoc -> method.addJavadoc("$L", javadoc));
        ...
        if (defaultValue instanceof ExpressionDef.Constant constant) {
            ...
            if (constant.type() instanceof TypeDef.Array arrayDef
                ...
                final var values = IntStream.range(0, Array.getLength(constantValue))
                    .mapToObj(i -> conversions.renderConstant(RenderScope.root(null),
    ...
    private TypeSpec.Builder getClassBuilder(ClassDef classDef) {
        ...
        classDef.getJavadoc().forEach(javadoc -> classBuilder.addJavadoc("$L", javadoc));
        ...
        if (staticInitializer != null) {
            RenderScope scope = RenderScope.root(null);
            scope.reassigns(JavaSourceRules.reassignedLocals(List.of(staticInitializer)));
            CodeBlock staticBlock;
            if (JavaSourceRules.containsReturn(staticInitializer)) {
                // Java has no return in an initializer: the body is a labelled block its returns break out of
                String label = INITIALIZER_LABEL;
                staticBlock = CodeBlock.builder()
                    .add("$L: {\n", label)
                    .indent()
                    .add(renderStatementCodeBlock(classDef, null, scope.breakingTo(label), staticInitializer, true))
                    .unindent()
                    .add("}\n")
                    .build();
            } else {
                staticBlock = renderStatementCodeBlock(classDef, null, scope, staticInitializer, true);
            }
    ...
    private TypeSpec.Builder getRecordBuilder(RecordDef recordDef) {
        ...
        recordDef.getJavadoc().forEach(javadoc -> classBuilder.addJavadoc("$L", javadoc));
        ...
        for (PropertyDef property : recordDef.getProperties()) {
            ...
            property.getJavadoc().forEach(javadoc -> componentBuilder.addJavadoc("$L", javadoc));
            for (AnnotationDef annotation : property.getAnnotations()) {
    ...
    private void addInnerTypes(List<ObjectDef> innerTypes, TypeSpec.Builder classBuilder, boolean isInterface) {
        for (ObjectDef innerType : innerTypes) {
            ...
            } else if (innerType instanceof ClassDef) {
                // The bytecode writer gives a member class no outer instance: a nested class of the model is a static
                // nested class, which a static method of the outer class can instantiate. A record, an enum and an
                // interface are implicitly static
                innerBuilder.addModifiers(Modifier.STATIC);
            }
    ...
    private void buildFields(ObjectDef objectDef, List<FieldDef> fields, TypeSpec.Builder builder) {
        for (FieldDef field : fields) {
            ...
            // An instance field, likewise, keeps `final` where every constructor assigns it exactly once
            boolean dropsFinal = field.getModifiers().contains(Modifier.FINAL)
                && (field.getModifiers().contains(Modifier.STATIC)
                ? field.getInitializer().isEmpty() && !keepsFinal(objectDef, field.getName())
                : !JavaSourceRules.keepsFinal(objectDef, field));
            Modifier[] modifiers = dropsFinal
            ...
            field.getJavadoc().forEach(javadoc -> fieldBuilder.addJavadoc("$L", javadoc));
    ...
    private void buildProperties(ObjectDef objectDef, TypeSpec.Builder builder) {
        for (PropertyDef property : objectDef.getProperties()) {
            ...
            property.getJavadoc().forEach(javadoc -> fieldBuilder.addJavadoc("$L", javadoc));
            builder.addField(
                fieldBuilder
                    .build()
            );
    ...
    private MethodSpec asMethodSpec(ObjectDef objectDef, MethodDef method) {
        ...
        method.getJavadoc().forEach(javadoc -> methodBuilder.addJavadoc("$L", javadoc));
        ...
        methodScope.reassigns(JavaSourceRules.reassignedLocals(statements));
    ...
    private void addAnnotationValue(AnnotationSpec.Builder builder, String memberName, Object value) {
        switch (value) {
            ...
            case Enum<?> anEnum ->
                // The declaring class: a constant with a body is of an anonymous subclass
                builder.addMember(memberName, "$T.$L", anEnum.getDeclaringClass(), anEnum.name());
            ...
            case ClassTypeDef typeDef ->
                builder.addMember(memberName, "$T.class", asClassType(typeDef));
    ...
    final TypeName asType(TypeDef typeDef, @Nullable ObjectDef objectDef) {
        return asType(typeDef, objectDef, null, false);
    }

    final TypeName asType(TypeDef typeDef, @Nullable ObjectDef objectDef, @Nullable MethodDef methodDef) {
        return asType(typeDef, objectDef, methodDef,
            methodDef != null && methodDef.getModifiers().contains(Modifier.STATIC));
    }

    final TypeName asType(TypeDef typeDef, @Nullable ObjectDef objectDef, boolean staticContext) {
        return asType(typeDef, objectDef, null, staticContext);
    }

    final TypeName asType(TypeDef typeDef,
    ...
    private CodeBlock renderStatement(@Nullable ObjectDef objectDef,
        ...
        switch (statementDef) {
            ...
            case StatementDef.Return aReturn -> {
                if (scope.yieldType() != null) {
                    // In the block of a switch expression case, a return of the model yields the value
                    if (aReturn.expression() == null) {
                        throw new IllegalStateException("Switch yield return has no value");
                    }
                    return CodeBlock.concat(CodeBlock.of("yield "),
                        renderStored(objectDef, methodDef, scope, scope.yieldType(), aReturn.expression()));
                }
                if (aReturn.expression() == null) {
                    // In an initializer, which Java has no return in, a return breaks out of the labelled block
                    return scope.breakLabel() != null ? CodeBlock.of("break $L", scope.breakLabel()) : CodeBlock.of("return");
                }
                ...
                if (scope.breakLabel() != null) {
                    throw new IllegalStateException("An initializer returns no value: " + aReturn.expression());
                }
                ...
                if (methodDef != null && isFunctional(returned) && !sameErasure(methodDef.getReturnType(), returned.type())) {
                    return CodeBlock.concat(CodeBlock.of("return "), withTargetType(objectDef, methodDef, returned,
                        renderExpression(objectDef, methodDef, scope, returned), methodDef.getReturnType()));
                }
                if (casts.isEmpty() && methodDef != null) {
                    // Written out: the raw type is the type the value has in the model, whose cast is dropped
                    List<List<TypeDef>> inferred = JavaExpressionRules.inferredResultCasts(methodDef.getReturnType(), returned, objectDef, methodDef);
                    if (!inferred.isEmpty()) {
                        return CodeBlock.concat(CodeBlock.of("return "),
                            renderConverted(objectDef, methodDef, scope, returned, inferred, true));
                    }
                }
                return CodeBlock.concat(CodeBlock.of("return "),
            ...
            case StatementDef.Assign assign -> {
                return CodeBlock.concat(
                    ...
                    CodeBlock.of(" = "),
                    renderStored(objectDef, methodDef, scope, assign.variable().type(), assign.expression())
                );
            }
            case StatementDef.PutField putField -> {
                return CodeBlock.concat(
                    ...
                    CodeBlock.of(" = "),
                    renderStored(objectDef, methodDef, scope, putField.field().type(), putField.expression())
                );
            }
            case StatementDef.PutStaticField putStaticField -> {
                ...
                return CodeBlock.concat(
                    target,
                    CodeBlock.of(" = "),
                    renderStored(objectDef, methodDef, scope, putStaticField.field().type(), putStaticField.expression())
                );
            }
            case StatementDef.DefineAndAssign assign -> {
                // A local named like a parameter, or like a local of an enclosing scope - the body of a lambda has
                // locals of its own in bytecode - is emitted under an allocated name, and its references remapped
                String name = assign.variable().name();
                // A keyword is a name in bytecode, not in source
                String emittedName = SourceVersion.isKeyword(name) ? scope.allocate(name + 1) : scope.isTaken(name) ? scope.allocate(name) : name;
                // In scope of its own initializer: a lambda in it cannot name a parameter like the variable
                scope.rename(name, emittedName);
                return CodeBlock.concat(
                    CodeBlock.of("$T $L", asType(assign.variable().type(), objectDef, methodDef), emittedName),
                    CodeBlock.of(" = "),
                    renderStored(objectDef, methodDef, scope, assign.variable().type(), assign.expression())
                );
            }
    ...
    private CodeBlock renderStatementCodeBlock(@Nullable ObjectDef objectDef,
        ...
        switch (statementDef) {
            ...
            case StatementDef.Try tryStatement -> {
                // A dead handler - of an exception caught before it, or of a checked one the body cannot throw - is
                // dropped, which javac rejects and the bytecode never reaches
                List<StatementDef.Try.Catch> catches = JavaSourceRules.liveCatches(tryStatement, objectDef);
                ...
                builder.unindent();
                int i = 0;
                for (StatementDef.Try.Catch aCatch : catches) {
                    String exceptionLocal = "e" + i++;
                    while (declaresField(objectDef, exceptionLocal) || scope.isTaken(exceptionLocal)
                        || JavaSourceRules.declaresLocal(aCatch.statement(), exceptionLocal)) {
            ...
            case StatementDef.Synchronized s -> {
                CodeBlock.Builder builder = CodeBlock.builder();
                builder.add(conversions.renderCapturedCopies(objectDef, methodDef, scope, s.monitor()));
            ...
            case StatementDef.If ifStatement -> {
                CodeBlock.Builder builder = CodeBlock.builder();
                builder.add(conversions.renderCapturedCopies(objectDef, methodDef, scope, ifStatement.condition()));
                builder.add("if (");
            ...
            case StatementDef.Switch aSwitch -> {
                CodeBlock.Builder builder = CodeBlock.builder();
                builder.add(conversions.renderCapturedCopies(objectDef, methodDef, scope, aSwitch.expression()));
                builder.add("switch (");
                ...
                for (Map.Entry<ExpressionDef.Constant, StatementDef> e : aSwitch.cases().entrySet()) {
                    builder.add("case ");
                    builder.add(conversions.renderConstant(scope, e.getKey()));
                    builder.add(" -> {\n");
                    builder.indent();
            ...
            case StatementDef.While aWhile -> {
                if (JavaSourceRules.isConstantFalse(aWhile.expression())) {
                    // The body of a loop over a constant `false` is unreachable, which javac rejects
                    return CodeBlock.of("");
                }
                CodeBlock.Builder builder = CodeBlock.builder();
                builder.add("while (");
            ...
            case StatementDef.Return aReturn when aReturn.expression() != null
                && TypeDef.VOID.equals(aReturn.expression().type()) && scope.yieldType() == null -> {
                ...
                CodeBlock.Builder builder = CodeBlock.builder()
                    .add(conversions.renderCapturedCopies(objectDef, methodDef, scope, statementDef))
                    .addStatement(renderExpression(objectDef, methodDef, scope, aReturn.expression()));
                if (!tailPosition && scope.breakLabel() != null) {
                    builder.addStatement("break $L", scope.breakLabel());
            ...
            case null, default -> {
                CodeBlock copies = statementDef == null ? CodeBlock.of("")
                    : conversions.renderCapturedCopies(objectDef, methodDef, scope, statementDef);
    ...
    final CodeBlock renderExpression(@Nullable ObjectDef objectDef,
                                       @Nullable MethodDef methodDef,
                                       RenderScope scope,
    ...
    final CodeBlock renderExpression(@Nullable ObjectDef objectDef,
        ...
        switch (expressionDef) {
            ...
            case ExpressionDef.NewInstance newInstance -> {
                return renderInvocation(objectDef, methodDef, scope, newInstance, () -> CodeBlock.concat(
                    ...
                    renderInvocationArguments(objectDef, methodDef, scope, newInstance.type(), MethodDef.CONSTRUCTOR,
                        ...
                        OverrideResolver.receiverArguments(newInstance.type(), objectDef,
                            MethodDef.constructor().addParameters(newInstance.parameterTypes()).build()),
                        newInstance.values()),
                    CodeBlock.of(")")
                ));
            }
            ...
            case ExpressionDef.NewArrayOfSize newArray -> {
                ...
                return CodeBlock.concat(arrayCreationCast(objectDef, methodDef, newArray.type()),
                    CodeBlock.of("new $T[$L]$L", asType(creationComponent(objectDef, methodDef, newArray.type()), objectDef, methodDef),
                        newArray.size(), "[]".repeat(newArray.type().dimensions() - 1)));
            }
            ...
            case ExpressionDef.Cast castExpressionDef -> {
                ExpressionDef exp = collapseNestedCasts(castExpressionDef.expressionDef(), castExpressionDef.type());
                ...
                if (TypeHierarchy.unwrap(castExpressionDef.type()) instanceof TypeDef.Primitive
                    && (exp instanceof ExpressionDef.IfElse || exp instanceof ExpressionDef.Switch)
                    && JavaExpressionRules.castsNumericBranches(JavaExpressionRules.resultsOf(exp), exp.type())) {
                    // A conditional of numeric results cast to a primitive is typed numerically, as Java types it: its
                    // results are not cast to the type of the model, which the cast could not unbox
                    CodeBlock conditional = exp instanceof ExpressionDef.IfElse ifElse
                        ? renderIfElse(objectDef, methodDef, scope, ifElse, false)
                        : renderSwitch(objectDef, methodDef, scope, (ExpressionDef.Switch) exp, false);
                    return CodeBlock.of("($T) $L", asType(castExpressionDef.type(), objectDef, methodDef), addParentheses(conditional));
                }
            ...
            case ExpressionDef.Constant constant -> {
                return conversions.renderConstant(scope, constant);
            }
            case ExpressionDef.InvokeInstanceMethod invokeInstanceMethod -> {
                return renderInvocation(objectDef, methodDef, scope, invokeInstanceMethod,
                    () -> renderInvokeInstanceMethod(objectDef, methodDef, scope, invokeInstanceMethod));
            }
            case ExpressionDef.InvokeStaticMethod staticMethod -> {
                return renderInvocation(objectDef, methodDef, scope, staticMethod, () -> CodeBlock.concat(
            ...
            case ExpressionDef.MathUnaryOperation mathOperation -> {
                ...
                ExpressionDef unwrapped = unwrapCasts(mathOperation.expression());
                if (unwrapped instanceof ExpressionDef.MathUnaryOperation || isNegativeNumericConstant(unwrapped)) {
                    // `--1` is a decrement, and `- -1` reads as one: the operand is grouped
                    operand = addParentheses(operand);
                }
                return CodeBlock.concat(CodeBlock.of(getMathOp(mathOperation)), operand);
            }
            case ExpressionDef.IfElse condition -> {
                return renderIfElse(objectDef, methodDef, scope, condition);
            }
            case ExpressionDef.Switch aSwitch -> {
                return renderSwitch(objectDef, methodDef, scope, aSwitch);
            }
            case ExpressionDef.SwitchYieldCase switchYieldCase -> {
                return renderSwitchYieldCase(objectDef, methodDef, scope, switchYieldCase);
            }
            case VariableDef variableDef -> {
    ...
    private CodeBlock renderNewArrayInitialized(@Nullable ObjectDef objectDef, @Nullable MethodDef methodDef, RenderScope scope, ExpressionDef.NewArrayInitialized newArray) {
        CodeBlock.Builder builder = CodeBlock.builder();
        builder.add(arrayCreationCast(objectDef, methodDef, newArray.type()));
        builder.add("new $T{", asType(TypeDef.array(creationComponent(objectDef, methodDef, newArray.type()), newArray.type().dimensions()), objectDef, methodDef));
    ...
    /**
     * An invocation, written as it renders - or, where an argument converts to an array of a variable of the invoked
     * method bounded by itself, which no inferred variable satisfies, through a helper declaring the variable.
     */
    private CodeBlock renderInvocation(@Nullable ObjectDef objectDef,
                                       @Nullable MethodDef methodDef,
                                       RenderScope scope,
                                       ExpressionDef invocation,
                                       Supplier<CodeBlock> rendering) {
        try {
            return rendering.get();
        } catch (JavaConversionRenderer.SelfBoundedArgument argument) {
            CodeBlock helper = conversions.renderSelfBoundedInvocation(objectDef, methodDef, scope, invocation, argument);
            if (helper == null) {
                throw argument;
            }
            return helper;
    ...
    private CodeBlock renderInvokeInstanceMethod(@Nullable ObjectDef objectDef, @Nullable MethodDef methodDef, RenderScope scope, ExpressionDef.InvokeInstanceMethod invokeInstanceMethod) {
        ...
        if (callMethod.isConstructor() && invokeInstanceMethod.instance() instanceof VariableDef.Super) {
            ...
            if (isFunctional(invokeInstanceMethod.instance())) {
                ...
                instance = addParentheses(withTargetType(objectDef, methodDef, invokeInstanceMethod.instance(), instance));
            } else if (!callMethod.isConstructor() && requiresMethodCallTargetParentheses(invokeInstanceMethod.instance())) {
        ...
        return CodeBlock.concat(
            ...
            renderInvocationArguments(objectDef, methodDef, scope,
                ownerOf(objectDef, methodDef, invokeInstanceMethod.instance().type(), callMethod.getName(),
                    callMethod.getParameters().stream().map(ParameterDef::getType).toList()),
                callMethod, invokeInstanceMethod.values()),
            CodeBlock.of(")")
        );
    }

    private CodeBlock renderIfElse(@Nullable ObjectDef objectDef, @Nullable MethodDef methodDef, RenderScope scope, ExpressionDef.IfElse condition) {
        return renderIfElse(objectDef, methodDef, scope, condition, true);
    }

    /**
     * @param asReference Whether numeric results are cast to the reference type of the model, where the expression is
     *                    used as a reference; a conditional cast to a primitive is typed numerically instead
     */
    private CodeBlock renderIfElse(@Nullable ObjectDef objectDef, @Nullable MethodDef methodDef, RenderScope scope, ExpressionDef.IfElse condition, boolean asReference) {
        ...
        boolean numeric = asReference && JavaExpressionRules.castsNumericBranches(JavaExpressionRules.resultsOf(condition), condition.type());
        return CodeBlock.concat(
            conditionBlock,
            CodeBlock.of(" ? "),
            renderBranch(objectDef, methodDef, scope, condition.type(), condition.ifExpression(), numeric),
            CodeBlock.of(" : "),
            renderBranch(objectDef, methodDef, scope, condition.type(), condition.elseExpression(), numeric)
        );
    }

    private CodeBlock renderSwitch(@Nullable ObjectDef objectDef, @Nullable MethodDef methodDef, RenderScope scope, ExpressionDef.Switch aSwitch) {
        return renderSwitch(objectDef, methodDef, scope, aSwitch, true);
    }

    private CodeBlock renderSwitch(@Nullable ObjectDef objectDef, @Nullable MethodDef methodDef, RenderScope scope, ExpressionDef.Switch aSwitch, boolean asReference) {
        CodeBlock.Builder builder = CodeBlock.builder();
        builder.add("switch (");
        builder.add(renderExpression(objectDef, methodDef, scope, aSwitch.expression()));
        builder.add(") {\n");
        builder.indent();
        boolean numeric = asReference && JavaExpressionRules.castsNumericBranches(JavaExpressionRules.resultsOf(aSwitch), aSwitch.type());
        for (Map.Entry<ExpressionDef.Constant, ? extends ExpressionDef> e : aSwitch.cases().entrySet()) {
            builder.add("case ");
            builder.add(conversions.renderConstant(scope, e.getKey()));
            builder.add(" -> ");
            ExpressionDef value = e.getValue();
            builder.add(renderBranch(objectDef, methodDef, scope, aSwitch.type(), value, numeric));
        ...
        if (aSwitch.defaultCase() != null) {
            builder.add("default");
            builder.add(" -> ");
            builder.add(renderBranch(objectDef, methodDef, scope, aSwitch.type(), aSwitch.defaultCase(), numeric));
    ...
    private CodeBlock renderSwitchYieldCase(@Nullable ObjectDef objectDef, @Nullable MethodDef methodDef, RenderScope scope, ExpressionDef.SwitchYieldCase switchYieldCase) {
        CodeBlock.Builder builder = CodeBlock.builder();
        builder.add("{\n");
        builder.indent();
        ...
        // The bytecode yields wherever the block returns, at any depth - in a try, a switch, a loop - and a block
        // that always throws yields nothing
        if (!cannotCompleteNormally(statement)) {
            throw new IllegalStateException("The block of a SwitchYieldCase has to end in a return or a throw. Found: " + statement);
        }
        builder.add(renderStatementCodeBlock(objectDef, methodDef, scope.yielding(switchYieldCase.type()), statement));
        builder.unindent();
        builder.add("}");
    ...
    private CodeBlock renderLambda(@Nullable ObjectDef objectDef, @Nullable MethodDef methodDef, RenderScope scope, Lambda lambda) {
        ...
        lambdaScope.reassigns(JavaSourceRules.reassignedLocals(implementation.getStatements()));
        // A captured local the enclosing body reassigns is read by the final copy made for the lambda
        JavaSourceRules.capturedReassignedLocals(lambda, scope::isReassigned).keySet().forEach(name -> {
            String copyName = scope.resolveCopy(name);
            if (copyName != null) {
                lambdaScope.rename(name, copyName);
            }
        });
        CodeBlock.Builder builder = CodeBlock.builder();
        builder.add("(");
        ...
        while (parameter.hasNext()) {
            ...
            // A `$L` argument: a parameter name can contain a `$`, read as a placeholder
        ...
        if (body != null) {
            ...
            for (int i = 0; i < statements.size(); i++) {
                builder.add(renderStatementCodeBlock(objectDef, implementation, lambdaScope, statements.get(i),
                    i == statements.size() - 1));
                if (cannotCompleteNormally(statements.get(i))) {
                    break;
                }
            }
    ...
    private CodeBlock renderMethodReference(@Nullable ObjectDef objectDef, @Nullable MethodDef methodDef, RenderScope scope, MethodReferenceExpression methodReference) {
        ...
        if (instance == null) {
            CodeBlock pinned = conversions.renderPinnedReference(objectDef, methodDef, scope, methodReference);
            return pinned != null ? pinned : CodeBlock.of("$T::$L", asType(methodReference.owner(), objectDef), name);
        }
    ...
    private CodeBlock renderStringConcatenation(@Nullable ObjectDef objectDef, @Nullable MethodDef methodDef, RenderScope scope, ExpressionDef.StringConcatenation concat) {
        ...
        return CodeBlock.concat(leftBlock, CodeBlock.of(" + "),
            conversions.renderConcatOperand(objectDef, methodDef, scope, concat.right(), true));
    }
    ...
    private CodeBlock renderBranch(@Nullable ObjectDef objectDef,
                                   ...
                                   ExpressionDef value,
                                   boolean numeric) {
        if (value instanceof ExpressionDef.SwitchYieldCase || isNullLiteral(value) || type.isPrimitive()) {
            return renderExpression(objectDef, methodDef, scope, value);
        }
        if (isFunctional(value)) {
            // A lambda takes its type from the expression, which an `Object` result does not give it
            CodeBlock rendered = renderExpression(objectDef, methodDef, scope, value);
            return sameErasure(type, value.type()) ? rendered : withTargetType(objectDef, methodDef, value, rendered);
        }
        TypeDef valueType = value.type();
        if (numeric && !type.equals(valueType)) {
            // Java types a conditional of numeric results numerically: `int` and `Integer` unbox, `Integer` and `Long`
            // widen to a `long` - where the bytecode boxes the primitive and keeps each result as it is
            return CodeBlock.concat(CodeBlock.of("($T) ", asType(type instanceof ClassTypeDef.Parameterized parameterized
                ? parameterized.rawType() : type, objectDef, methodDef)), renderCastOperand(objectDef, methodDef, scope, value));
        }
        if (valueType.isPrimitive() || !JavaExpressionRules.requiresImplicitInvocationCast(type, valueType)) {
            return renderExpression(objectDef, methodDef, scope, value);
        }
     ...
     */
    private CodeBlock renderStored(@Nullable ObjectDef objectDef,
                                   @Nullable MethodDef methodDef,
                                   RenderScope scope,
                                   TypeDef targetType,
                                   ExpressionDef value) {
        if (isNullLiteral(value) || TypeDef.VOID.equals(targetType) || value.type().equals(TypeDef.VOID)) {
            return renderExpression(objectDef, methodDef, scope, value);
        }
        if (isFunctional(value)) {
            CodeBlock rendered = renderExpression(objectDef, methodDef, scope, value);
            return sameErasure(targetType, value.type()) ? rendered : withTargetType(objectDef, methodDef, value, rendered, targetType);
        }
        TypeDef sourceType = sourceTypeOf(value, methodDef, objectDef);
        List<List<TypeDef>> casts = returnCasts(targetType, sourceType.equals(value.type()) ? value.type() : sourceType,
            sourceType, objectDef, methodDef);
        if (casts.isEmpty()) {
            // Written out: the raw type is the type the value has in the model, whose cast is dropped
            List<List<TypeDef>> inferred = JavaExpressionRules.inferredResultCasts(targetType, value, objectDef, methodDef);
            if (!inferred.isEmpty()) {
                return renderConverted(objectDef, methodDef, scope, value, inferred, true);
            }
        }
        return renderConverted(objectDef, methodDef, scope, value, casts);
    }
    ...
    private CodeBlock renderInvocationArguments(@Nullable ObjectDef objectDef,
        ...
        ObjectDef generatedOwner = OverrideResolver.definitionOf(owner, objectDef);
        if (signature == null && generatedOwner != null && methodName != null && sameArityParameterTypes != null
            && generatedOwner.getMethods().stream().noneMatch(method -> method.getName().equals(methodName)
            && method.getParameters().size() == sameArityParameterTypes.size())) {
            // A method a generated class inherits from a compiled one - `add` of an `ArrayList<String>` it extends
            signature = JavaExpressionRules.inheritedSignature(generatedOwner, methodName, sameArityParameterTypes);
        }
        ...
        boolean generated = signature == null && generatedOwner != null;
        ...
        boolean overloaded = methodName != null && sameArityParameterTypes != null
            && hasApplicableOverload(owner, OverrideResolver.definitionOf(owner, objectDef), methodName, sameArityParameterTypes,
            values.stream().map(value -> isNullLiteral(value) || isFunctional(value) ? null : sourceTypeOf(value, enclosingMethod, objectDef)).toList());
        return IntStream.range(0, values.size())
            .mapToObj(i -> {
                ExpressionDef value = values.get(i);
                if (sameArityParameterTypes != null) {
                    TypeDef declaredType = declaredTypes != null && declaredTypes.size() == values.size() ? declaredTypes.get(i) : null;
                    TypeDef paramType = JavaExpressionRules.receiverLowerBound(
                        receiverBound(sameArityParameterTypes.get(i), declaredType, inferred, receiverArguments),
                        declaredType, inferred, receiverArguments);
                    ...
                    if (vararg) {
                        return conversions.renderVarargsValue(objectDef, enclosingMethod, scope, paramType, inferred, value);
                    }
                    if (isFunctional(value)) {
                        ...
                        return overloaded || !sameErasure(paramType, value.type())
                    ...
                    if (intersection != null) {
                        TypeDef.Array array = (TypeDef.Array) TypeHierarchy.unwrap(paramType);
                        return conversions.renderIntersectionArray(objectDef, enclosingMethod, scope, value, intersection,
                            array.dimensions(), ((TypeDef.TypeVariable) TypeHierarchy.unwrap(array.componentType())).name());
                    }
                    ...
                    if (sibling != null) {
                        ...
                        return CodeBlock.concat(CodeBlock.of("($T) ", asType(sibling, objectDef, enclosingMethod)),
    ...
    /**
     * The cast of an array created of the erasure of a type variable to the array of the variable, or nothing.
     */
    private CodeBlock arrayCreationCast(@Nullable ObjectDef objectDef, @Nullable MethodDef methodDef, TypeDef.Array array) {
        return TypeHierarchy.unwrap(array.componentType()) instanceof TypeDef.TypeVariable
            ? CodeBlock.of("($T) ", asType(array, objectDef, methodDef)) : CodeBlock.of("");
    }
    ...
    /**
     * A lambda or a reference with its functional interface as its type, and, where it is returned or stored as a
     * type variable bounded by the interface, cast to the variable after it - {@code (F) (Function<Object, Object>)}
     * - an unchecked conversion, as the verifier sees the interface.
     */
    private CodeBlock withTargetType(@Nullable ObjectDef objectDef,
                                     @Nullable MethodDef methodDef,
                                     ExpressionDef functional,
                                     CodeBlock rendered,
                                     TypeDef targetType) {
        CodeBlock typed = withTargetType(objectDef, methodDef, functional, rendered);
        return TypeHierarchy.unwrap(targetType) instanceof TypeDef.TypeVariable
            ? CodeBlock.of("($T) $L", asType(targetType, objectDef, methodDef), typed) : typed;
    }

    final CodeBlock renderCastOperand(@Nullable ObjectDef objectDef,
                                        @Nullable MethodDef methodDef,
                                        RenderScope scope,
                                        ExpressionDef value) {
        CodeBlock rendered = renderExpression(objectDef, methodDef, scope, value);
    ...
    final CodeBlock addParentheses(CodeBlock rendered) {
        return CodeBlock.concat(
            CodeBlock.of("("),
            rendered,
            CodeBlock.of(")")
        ...
        switch (expressionDef) {
            ...
            case ExpressionDef.EqualsStructurally equalsStructurally -> {
                ...
                if (leftType.isPrimitive() || rightType.isPrimitive()) {
                    // The bytecode compares the primitive with the other operand converted to it
                    return renderEqualsReferentially(objectDef, methodDef, scope, asPrimitiveOperand(left, rightType), asPrimitiveOperand(right, leftType));
                }
            ...
            case ExpressionDef.NotEqualsStructurally notEqualsStructurally -> {
                ...
                TypeDef rightType = right.type();
                if (leftType.isPrimitive() || rightType.isPrimitive()) {
                    return renderNotEqualsReferentially(objectDef, methodDef, scope, asPrimitiveOperand(left, rightType), asPrimitiveOperand(right, leftType));
                }
    ...
    private CodeBlock renderVariable(@Nullable ObjectDef objectDef, @Nullable MethodDef methodDef, RenderScope scope, VariableDef variableDef) {
        switch (variableDef) {
            ...
            case VariableDef.Local localVariableDef -> {
                String emittedName = scope.resolveRename(localVariableDef.name());
                return CodeBlock.of("$L", emittedName == null ? localVariableDef.name() : emittedName);
            }
