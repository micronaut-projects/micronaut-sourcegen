...
/**
 * The resolved bridge descriptors, independent of the bytecode that is later written for them.
 */
class BridgeResolverTest {
    ...
    private static List<String> descriptors(ObjectDef objectDef, MethodDef methodDef) {
        return BridgeResolver.resolve(objectDef, methodDef).stream()
            .map(bridge -> bridge.parameterTypes().stream()
                .map(t -> TypeUtils.getType(t, null).getDescriptor())
                .reduce("", String::concat)
                .transform(parameters -> "(" + parameters + ")" + TypeUtils.getType(bridge.returnType(), null).getDescriptor()))
            .sorted()
            .toList();
    }
    ...
    private static final class ExpressionDefs {
        ...
        static io.micronaut.sourcegen.model.ExpressionDef constant() {
            return io.micronaut.sourcegen.model.ExpressionDef.constant("value");
        }
    }
}
